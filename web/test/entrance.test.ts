...
import { entranceFrame, flowPoint, followerX, introQuality, mainBall, smooth, waveRect, ENTRANCE_DURATION, ENTRANCE_TIMING, type Rect } from '../src/components/Entrance/timeline'
...
describe('football entrance continuity', () => {
  ...
  it('slows the followers into the live light flow without a speed discontinuity', () => {
    ...
    expect(velocity(.9)).toBeCloseTo(.32, 3)
    expect(velocity(2.8)).toBeCloseTo(.052, 3)
    expect(velocity(2.8 - .001)).toBeCloseTo(velocity(2.8 + .001), 3)
  })
  ...
  it('follows a curved launch and preserves the idle position on click', () => {
    ...
    const from = mainBall(.30, 0, viewport), mid = mainBall(.70, 0, viewport), to = mainBall(1.1, 0, viewport)
  ...
  it('lands on exactly the same field and bounds as the living hero', () => {
    expect(waveRect(viewport, hero, ENTRANCE_TIMING.handoffStart)).toEqual(hero)
    for (const layer of [0, 1, 2]) {
      for (const x of [0.1, 0.4, 0.8]) {
        const p = flowPoint(x, .2, layer, ENTRANCE_TIMING.handoffStart, 9.7, hero)
  ...
  it('has no position jump at any phase boundary', () => {
    for (const t of [.3, 1.1, 2, 2.6, 2.8, 3.5, 3.52, 4.15, 4.5]) {
  ...
  it('settles the geometry before crossfading, then releases the live site at 4.5 seconds', () => {
    expect(entranceFrame(0).hero).toBe(0)
    expect(entranceFrame(2.8).hero).toBe(0)
    expect(entranceFrame(3.6).hero).toBe(1)
    expect(ENTRANCE_TIMING.handoffStart).toBeGreaterThanOrEqual(ENTRANCE_TIMING.settleEnd)
    expect(entranceFrame(ENTRANCE_DURATION - .001).complete).toBe(false)
    expect(entranceFrame(4.5)).toMatchObject({ complete: true, hero: 1, content: 1, balls: 0 })
  ...
  it('keeps the idle face still and launches the main ball beyond the screen', () => {
    ...
    const fired = mainBall(1.1, 0, viewport)
