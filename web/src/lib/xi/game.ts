...
export function guessPlayer(match: XiMatch, state: XiState, side: Side, number: number, typed: string): { state: XiState; error?: GuessError } {
  ...
  const done = guess === player.word ? 'solved' : guesses.length >= MAX_TRIES ? 'failed' : null
  ...
  try {
    ...
    for (const side of ['home', 'away'] as Side[]) {
      for (const [number, sl] of Object.entries(s.teams[side] ?? {})) {
        ...
        if (!p || !Array.isArray(sl.guesses) || sl.guesses.length > MAX_TRIES) return null
        if (sl.guesses.some((g) => typeof g !== 'string' || g.length !== p.word.length)) return null
        const done = sl.guesses.includes(p.word) ? 'solved' : sl.guesses.length >= MAX_TRIES ? 'failed' : null
...
export function dailyMatch(matches: XiMatch[], day: number): XiMatch {
  return dailyList(matches, new Date(day * DAY))!
}
